...
describe("probeModel", () => {
  ...
  it("treats an error body on a 200 as a refusal too", async () => {
    stubFetch(() => new Response(JSON.stringify({ error: { message: "Provider returned error", metadata: { raw: "region blocked" } } }), { status: 200 }))
    const probe = await probeModel("blocked/model")
    expect(probe.ok).toBe(false)
    if (!probe.ok) expect(probe.error).toBe("Provider returned error")
  })

  it("sends a one-token request and caches a success, never a failure", async () => {
    const calls = stubFetch((_url, init) => {
      const body = JSON.parse(String(init?.body))
      expect(body).toMatchObject({ model: "fine/model", max_tokens: 1 })
