...
Author workflows yourself and push them with `bun run jig edit <id> --file=<file>` (creates the jig if it does not exist; typechecked, pending until approved); `bun run jig types` pulls the instance's connection types into `.jig/connections/` to write against. There is no in-server writer; the dashboard offers copy-ready prompts for a coding agent, and replying to a failure email edits the jig. All of these act on your deployed instance when you have one and take `--local` for this machine. SQLite is the only source of truth for jig code; nothing reads jig source off disk.
